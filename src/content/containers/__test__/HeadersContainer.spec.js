...
import HeadersReduxConnect from '../HeadersReduxConnect'

describe('>>> Connected Components',()=>{
    const jsonState = JSON.parse(fs.readFileSync(path.join(__dirname) + "\\mockStoreState.json"))
    ...
    it('+++ check props are updated properly with new state', () => {
        ...
        const { selectedHeaders, preHeaderCount, toggleCapture } = preHeaders
        ...
        expect(wrapper.prop('toggleCapture')).toBe(toggleCapture)
    });
});